...
public class Main {
	...
	public static void main(String[] args) {
		...
		for (String s: llista) {
			paisos.add(new Pais(s));
		}
		
		System.out.println("Criteri en una funció clàssica");
		mostra(paisos, 'A', 7);
		System.out.println();
		
		System.out.println("Criteri en una classe local");
		mostraPaisos(paisos, new Seleccio());
		System.out.println();
		
		System.out.println("Usant classes anònimes");
		mostraPaisos(paisos, new Filtrable() {
		...
		System.out.println("Usant lambda");
		mostraPaisos(paisos, p -> p.getPrimCar()=='A' && p.getLlargada()==7);
		
		
	}

	public static void mostra(List<Pais> paisos, char lletra, int llargada) {
	    for (Pais p : paisos) {
	    	if(p.getPrimCar()==lletra && p.getLlargada()==llargada ) {
		    	p.mostraPais();
	    	}
	    }
	}
